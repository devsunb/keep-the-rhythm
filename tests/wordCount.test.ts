...
import type { Language } from "../src/defs/types";
...
describe("Word counting", () => {
	...
	test("counts Korean words", () => {
		expect(
			getLanguageBasedWordCount("안녕하세요 테스트", defaultScripts),
		).toBe(2);
		expect(getLanguageBasedWordCount("ㅋㅋ ㅜㅜ", defaultScripts)).toBe(2);
		expect(
			getLanguageBasedWordCount("한글ㅋㅋ123 테스트", defaultScripts),
		).toBe(2);
	});

	test("handles mixed scripts correctly", () => {
		expect(getWordCount("Hello 你好 World こんにちは", regex)).toBe(9); // 2 Latin + 2 CJK + 5 Japanese
		expect(getWordCount("Test123 테스트 test", regex)).toBe(3); // 1 Latin+num + 1 Korean + 1 Latin
		expect(getWordCount("Hello世界test123", regex)).toBe(4); // 1 Latin + 2 CJK + 1 Latin+num
		expect(getWordCount("Angular框架", regex)).toBe(3);
		expect(getWordCount("React테스트", regex)).toBe(1);
	});
